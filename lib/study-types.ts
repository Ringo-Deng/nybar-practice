...
export type SourceImage={src:string;alt:string;sourcePage:number;width?:number;height?:number};
...
export type Explanation={kind?:'publisher-original';publisherReference?:{text:string;book:string;chapters:number[]};originalPdfPages?:number[];images?:SourceImage[];reasoningSteps?:{title:string;detail:string}[];textbookExample?:{title:string;facts:string;lesson:string;connection:string};textbookReferences?:TextbookReference[];concepts?:{term:string;detail:string}[];answer:string;topic:string;topicZh?:string;en:string;zh:string;ruleEn:string;ruleZh:string;warning:string;options:Record<string,{en:string;zh:string}>;source:string;sourceUrl:string};
export type Question={sourceId:string;sourceSet?:string;sourceSetLabel?:string;images?:SourceImage[];sourceSession?:number;sourcePages?:number[];sourceTitle?:string;subjectId:string;chapterId?:string;relatedChapterIds?:string[];topicTags?:string[];knowledge?:KnowledgeReview;id:string;number:number;stem:string;stemZh:string;ask:string;askZh:string;options:Option[];explanation?:Explanation};
