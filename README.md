...
在题库分类中选择 UWorld 和科目，即可开始或继续练习。既有答题记录保留，历史练习仍可从学习记录中打开。题目保留英文原文、原解析、图表和 PDF 页码；可在题目右上角点击“显示中文”，翻译题干、提问和全部选项；提交答案后，原书解析另有“显示中文 / 显示英文”切换。图表保留原图，其中嵌入的文字不自动翻译。原 PDF 可在教材中阅读，也可从解析打开对应页。
...
翻译按需在本机运行：优先使用浏览器内置英译中能力；不支持时，使用后台翻译模型，首次需联网下载约 115 MB 模型（另有约 22 MB 运行资源）。下载和翻译期间可继续答题或切题，失败时可重试。无需账号或 API 密钥，题文不发送给翻译服务器。

中文标注为机器翻译，供阅读辅助；英文原文、原书答案、PDF 和图表保持原样。长解析分段处理，保留原段落。已有题库中文优先展示；自动译文使用独立、有限容量的本机缓存，原文变化会重新翻译。缓存可能被浏览器清理，首次模型下载也可能受网络限制；本功能不代表全库已预先人工翻译或校对。浏览器翻译接口说明及备用模型来源分别见 [Chrome Translator API](https://developer.chrome.com/docs/ai/translator-api) 和 [Xenova/opus-mt-en-zh](https://huggingface.co/Xenova/opus-mt-en-zh)。
