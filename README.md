...
- 已导入用户提供的 **2025UWorld MBE QBank.pdf**：2,003 题，按七科拆成 **46 组，每组不超过 50 题**。
- 按传统 UBE 的 MBE 七科分类：民事诉讼法、宪法、合同法、刑法与刑事诉讼、证据法、不动产法、侵权法。
- UWorld 分类含正文 1,803 题及原文件的 NCBE 200 题附录。附录按实际考查科目归入七科，并单独标识批次。其他来源分类仍待导入。
- 保留四选一练习、中英文本展示、逐题解析、错题重练、学习记录和记忆卡片。
- 支持导入个人 PDF 教材、连续阅读、高亮及笔记。

在题库分类中选择 UWorld、科目和批次，即可开始或继续练习。题目保留英文原文、原解析、图表和 PDF 页码；本次未添加中文翻译。原 PDF 可在教材中阅读，也可从解析打开对应页。
...
完整核对记录、每题页码、答案识别依据、批次及抽查记录位于 [`data/uworld-2025/`](data/uworld-2025/)。来源文件 SHA-256 已记录；源 PDF 保持原样。抽查覆盖七科及特殊版式共 31 题，题库结构和全部媒体引用另作自动检查。
