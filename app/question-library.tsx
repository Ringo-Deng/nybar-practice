...
export function QuestionLibrary({data,busy,onStart,onResume}:{data:StudyData;busy:boolean;onStart:(subjectId?:string,sourceId?:string,chapterId?:string,sourceSet?:string)=>void;onResume:(session:Session)=>void}){
 ...
 const exactSession=(items:typeof sourceQs)=>{const ids=new Set(items.map(item=>item.id));return data.sessions.find(s=>s.status==='active'&&s.mode==='practice'&&s.questionIds.length===ids.size&&s.questionIds.every(id=>ids.has(id)));};
 const batchMap=new Map<string,{id:string;label:string;items:typeof qs}>();
 for(const question of qs){if(!question.sourceSet)continue;const existing=batchMap.get(question.sourceSet);if(existing)existing.items.push(question);else batchMap.set(question.sourceSet,{id:question.sourceSet,label:question.sourceSetLabel||`第 ${batchMap.size+1} 批`,items:[question]});}
 ...
 return <div className="library-page">
  ...
  {subject&&<section id="subject-detail" className="subject-detail" aria-live="polite"><div className="subject-detail-heading"><div><div className="eyebrow">{subject.group} / {sourceName}</div><h2>{subject.zh}<span>{qs.length} 题</span></h2></div>{qs.length>0&&<div className="subject-actions">{batches.length?<Button disabled={busy} onClick={()=>{const element=document.getElementById('practice-batches');element?.scrollIntoView({block:'start'});element?.focus({preventScroll:true});}}>选择练习批次<ArrowRight size={16}/></Button>:<>{resume&&<Button disabled={busy} variant="outline" onClick={()=>onResume(resume)}>继续上次练习</Button>}<Button disabled={busy} onClick={()=>onStart(subject.id,sourceArg)}>开始本科练习<ArrowRight size={16}/></Button></>}</div>}</div>
   {!qs.length&&<div className="subject-empty-state"><div><h3>本科题目待导入</h3><p>{subject.scope}</p></div></div>}
   {!!batches.length&&<section id="practice-batches" className="practice-batches" aria-labelledby="practice-batches-title" tabIndex={-1}><div className="practice-batches-heading"><h3 id="practice-batches-title">选择练习批次</h3><p>按原题顺序分组，每批最多 50 题，可随时保存并继续。</p></div><div className="practice-batch-grid">{batches.map(batch=>{const previous=exactSession(batch.items);const practicedCount=batch.items.filter(item=>practiced.has(item.id)).length;return <article className="practice-batch-card" key={batch.id}><div className="practice-batch-copy"><h4>{batch.label}</h4><p>{sourceById(batch.items[0].sourceId)?.name} · {batch.items.length} 题</p><small>{practicedCount} / {batch.items.length} 题已练</small></div><div className="practice-batch-actions">{previous&&<Button variant="outline" size="sm" disabled={busy} onClick={()=>onResume(previous)}>继续</Button>}<Button size="sm" disabled={busy} onClick={()=>onStart(subject.id,sourceArg,undefined,batch.id)}>{previous?'重新开始':'开始练习'}<ArrowRight size={15}/></Button></div></article>;})}</div></section>}
   {!batches.length&&qs.some(q=>q.chapterId)&&!!chapterGroups.length&&<div className="chapter-section compact-chapter-section">{chapterGroups.map(group=><section className="chapter-book" key={group.bookId} aria-label={group.bookZh}><div className="chapter-grid">{group.items.map(c=>{const items=filterQuestions(qs,{chapterId:c.id});const previous=exactSession(items);return <div className={`chapter-card ${items.length?'':'chapter-empty'}`} key={c.id}><span className="chapter-number">{String(c.number).padStart(2,'0')}</span><div className="chapter-copy"><h4>{c.zh}</h4><p lang="en">{c.en}</p><small>{items.length?`${items.filter(q=>practiced.has(q.id)).length} / ${items.length} 题已练`:'暂无题目'}</small></div><div className="chapter-actions">{previous&&<Button size="sm" variant="outline" disabled={busy} onClick={()=>onResume(previous)}>继续</Button>}<Button size="sm" variant="outline" disabled={busy||!items.length} onClick={()=>onStart(subject.id,sourceArg,c.id)} aria-label={`练习${group.bookZh}第${c.number}章${c.zh}`}>{items.length?`练习 ${items.length} 题`:'待导入'}</Button></div></div>;})}</div></section>)}</div>}
  </section>}
  <p className="library-footnote">当前练习范围：传统 UBE 的 MBE 选择题。题量以各分类显示为准；题目与解析按上传版本保留，未做现行法更新。<br/><a href="https://www.ncbex.org/exams/mbe/preparing-mbe" target="_blank" rel="noreferrer">NCBE 考试范围与官方样题</a></p>
 </div>;
}
